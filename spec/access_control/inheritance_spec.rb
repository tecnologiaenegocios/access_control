  ...
  describe Inheritance do
    ...
    describe ".parent_models_and_options" do
      ...
      describe "for each reflected model" do
        ...
        let(:record1) { mock('record 1', :pk => 'id 1') }
        let(:record2) { mock('record 2', :pk => 'id 2') }
        ...
        it "checks if the reflected model includes Restriction" do
          lambda {
            model.parent_models_and_options
          }.should raise_exception(InvalidInheritage)
        end

        describe "when Restriction is included in the reflected model" do
          ...
          it "runs within the block without restriction" do
            manager.instance_eval do
              def without_query_restriction
                yielded(yield)
              end
            ...
            manager.should_receive(:yielded).
              with([[reflected_model, :parent, Set.new(['id 1', 'id 2'])]])
            model.parent_models_and_options
          end
